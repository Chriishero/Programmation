...
class LogisticRegression:
    ...
    def fit(self, X, y):
        ...
        elif self.method == "newtow method":
            self.newtow_method(X, y)

    def predict(self, X):
        X = np.array(X)
        return (self.sigmoid(X) >= 0.5).astype(int)
    
model = LogisticRegression(n_iteration=50, learning_rate=0.1, method="gradient descent")
model.fit(X, y)
